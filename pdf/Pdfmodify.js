...
const PDFModify = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      let query = supa.from("medicalReport").select("*").eq("msds", msds); // 🔹 Corregido "mdsd" -> "msds"
      ...
      if (nCaso.trim() !== "") {
        ...
        if (!isNaN(nCasoParsed)) {
          ...
          Alert.alert("Error", "El número de caso debe ser un número válido.");
          return;
        }
      }
      ...
      if (error) {
        Alert.alert("Error", "No se pudo obtener los reportes.");
        return;
      }
